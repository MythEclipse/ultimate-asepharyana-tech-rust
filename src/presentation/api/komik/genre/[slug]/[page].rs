...
use crate::shared::utils::scraping::{selector, text_from_or, attr_from_or, text, attr};
...
// Removed GenreQuery struct as it is now path-based
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 200, description = "Handles GET requests for the /api/komik/genre/[slug]/[page] endpoint.", body = serde_json::Value),
...
pub async fn page(
    ...
    info!("komik genre request: {}, page: {}", slug, page);

    let cache_key = format!("komik:genre:{}:{}:v2", slug, page);
    let cache = Cache::new(&app_state.redis_pool);

    let response = cache
        .get_or_set(&cache_key, CACHE_TTL, || async {
            let (mut komik_list, pagination) = fetch_genre_komik(&slug, page)
                .await
...
async fn fetch_genre_komik(
    ...
    let url = if page == 1 {
        format!("{}/genre/{}/", get_komik_api_url(), slug)
    } else {
        format!(
            "{}/genre/{}/page/{}/",
            get_komik_api_url(),
...
pub fn register_routes(router: Router<Arc<AppState>>) -> Router<Arc<AppState>> {
    router.route("/api/komik/genre/{slug}/{page}", axum::routing::get(page))
}