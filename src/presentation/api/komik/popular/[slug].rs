...
use crate::shared::utils::scraping::{attr, attr_from_or, selector, text, text_from_or};
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 200, description = "Retrieves details for a specific popular by slug.", body = serde_json::Value),
...
pub fn register_routes(router: Router<Arc<AppState>>) -> Router<Arc<AppState>> {
    router.route("/api/komik/popular/{slug}", axum::routing::get(popular))
}