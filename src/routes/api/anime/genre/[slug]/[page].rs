...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 200, description = "Handles GET requests for the /api/anime/genre/[slug]/[page] endpoint.", body = serde_json::Value),
...
pub async fn page(
    ...
    info!("Handling request for genre: {}, page: {}", slug, page);

    let cache_key = format!("anime:genre:{}:{}", slug, page);
    let cache = Cache::new(&app_state.redis_pool);

    let response = cache
        .get_or_set(&cache_key, CACHE_TTL, || async {
            let (anime_list, pagination) = fetch_genre_anime(&slug, page)
                .await
...
async fn fetch_genre_anime(
    ...
    let url = if page == 1 {
        format!("{}/genres/{}/", get_otakudesu_url(), slug)
    } else {
        format!(
            "{}/genres/{}/page/{}/",
            get_otakudesu_url(),
