...
//use axum::{extract::Query, response::IntoResponse, routing::get, Json}; Handler for the komik manhua slug endpoint.
...
// Removed QueryParams as pagination is now path-based
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 200, description = "Retrieves details for a specific manhua by slug.", body = serde_json::Value),
...
pub fn register_routes(router: Router<Arc<AppState>>) -> Router<Arc<AppState>> {
    router.route("/api/komik/manhua/{slug}.rs", axum::routing::get(index))
}